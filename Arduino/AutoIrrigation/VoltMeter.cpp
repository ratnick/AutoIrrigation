...
void VoltMeterClass::init(int _pinNbr, char _name[], int _muxChannel, SensorHandlerClass::SensorType _sensorType, float _lastSummarizedReading)
{
	...
	L::LogLinef(3, __FUNCTION__, "MUX channel:%d analog pin:%d   name:%s  lastSummarizedReading:%f", muxChannel, pinNbr, name, lastSummarizedReading);
...
float VoltMeterClass::ReadVoltage() {
	...
	float sumRes = 0.0;
	L::LogLinef(3, __FUNCTION__, "READING FROM analog MUX channel %d", muxChannel);
	AnalogMux.OpenChannel(muxChannel);
	float res = analogRead(pinNbr);
	AnalogMux.CloseMUXpwr();
	L::LogLinef(2, __FUNCTION__, "Value = %f   converted = %f", res, (res * factor));
	L::LogLinef(5, __FUNCTION__, "Factor = %f", factor);
